...
0x00029C:
    self.image_index = 1s
0x0002A8:
    call (draw_sprite_part[]:int32 (+ self.y self.t) (+ self.x self.l) (- self.h self.t) (- self.w self.l) self.t self.l self.image_index (var 105s))
    push -5s
    call (draw_sprite_part[]:int32 (+ 2s:idealborder 6s) (+ self.x self.l) (- self.h self.t) (- self.w self.l) self.t self.l self.image_index (var 106s))
0x0003A4:
    push self.x
...
0x00044C:
    push (var 0s)
    call (draw_rectangle[]:int32 -5s (+ 2s:idealborder 10s) (+ self.x 9s) self.y (+ self.x 3s))
0x0004A0:
    if !(< (abs[]:int32 (- 743.x self.x)) 15s) goto 0x0004DC
0x0004C8:
    push (< global.invc 1s)
    goto 0x0004E0
0x0004DC:
...
0x0004E4:
    push (var 1s)
    if !(bool (collision_rectangle[]:int32 (var 0s) (var 743s) -5s (+ 2s:idealborder 10s) (+ self.x 9s) self.y (+ self.x 3s))) goto 0x000560
0x00054C:
    call (event_user[]:int32 (var 1s))
0x000560:
    push self.x
    if !(< -5s (- 0s:idealborder 10s)) goto 0x00059C
0x000588:
    push (< self.hspeed 0s)
    goto 0x0005A0
0x00059C:
...
0x0005B0:
    push self.x
    if !(> -5s (+ 1s:idealborder 10s)) goto 0x0005EC
0x0005D8:
    push (> self.hspeed 0s)
    goto 0x0005F0
0x0005EC:
